	...
	private static final class MediaDeviceMapper implements RowMapper<MediaDeviceData> {
		public String mediaDeviceSchema() {
			...
			return "SELECT ifnull(a.id, oh.id) AS deviceId," +
					...
					"cb.balance_amount as balanceAmount ,cc.code as currency  " +
					"from m_client c " +
					...
					"left join m_currency cc on (cc.country_code=ca.country ) " +
		...
		public String MediaDeviceBasedOnCientIdSchema() {
			...
			return "SELECT null as deviceId,c.id clientId,cv.code_value AS clientType,cv.id AS clientTypeId," +
					"cb.balance_amount as balanceAmount ,cc.code as currency from m_client c left join m_code_value cv on (c.category_type=cv.id )" +
					...
					" left join m_currency cc on (cc.country_code=ca.country ) WHERE c.id = ? group by c.id";
