...
public class FinancialTransactionApiResource {
	...
	public String retrieveSampleData(@PathParam("clientId") final Long clientId,@Context final UriInfo uriInfo, @QueryParam("type") final String type, 
		@QueryParam("officeId") final Long officeId, @QueryParam("limit") final Integer limit, @QueryParam("offset") final Integer offset)	{
	context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
	final SearchSqlQuery searchFinancialTransaction =SearchSqlQuery.forSearch(null, offset,limit );
	Page<FinancialTransactionsData> transactionData = this.billMasterReadPlatformService.retrieveSampleData(searchFinancialTransaction,clientId,type, officeId);
	return this.toApiJsonSerializer.serialize(transactionData);    
	}
	...
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public String retrieveOtpData(@Context final UriInfo uriInfo, @QueryParam("type") final String type,
	 @QueryParam("limit") final Integer limit, @QueryParam("offset") final Integer offset, @QueryParam("otp") final String otp, @QueryParam("officeId") final Long officeId)	{
	context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
	final SearchSqlQuery searchFinancialTransaction =SearchSqlQuery.forSearch(null, offset,limit );
	Page<FinancialTransactionsData> transactionData = this.billMasterReadPlatformService.retrieveOtpData(searchFinancialTransaction,type, otp, officeId);
	return this.toApiJsonSerializer.serialize(transactionData);    
	}
