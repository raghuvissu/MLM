...
public class BillMasterReadPlatformServiceImplementation implements BillMasterReadPlatformService {
	...
	private static final class FinancialTransactionsMapper implements
		...
		public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			return new FinancialTransactionsData(null, transactionId, transDate, transactionType, null, null, amount, null, 
					transactionCategory, false, planCode, description, clientName, rate, status, note, null, null);
		}
	...
	private static final class FinancialInvoiceTransactionsMapper implements
		...
		public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum)
			...
			return new FinancialTransactionsData(null, transactionId, transDate, transactionType, debitAmount, creditAmount, null, userName, 
					transactionCategory, flag, null, null, clientName, rate, status, note, null, null);
		}
	...
	private static final class BillStatmentMapper implements RowMapper<BillDetailsData> {
		...
		@Override
		public Page<FinancialTransactionsData> retrieveSampleData(final SearchSqlQuery searchTransactionHistory, final Long clientId, final String type, final Long officeId) {
			final FinancialTypeMapper financialTypeMapper = new FinancialTypeMapper();
		        ...
		        if(officeId != null){
		        	sqlBuilder.append(financialTypeMapper.financialTypeSchemaByOffice()).append(" and office_id = "+ officeId +"  and transType like '%" + type + "%' ");
		        }else{
		        	sqlBuilder.append(financialTypeMapper.financialTypeSchema()).append(" and transType like '%" + type + "%'");
		        }
		...
		@Override
		public Page<FinancialTransactionsData> retrieveOtpData(final SearchSqlQuery searchTransactionHistory, final String type, String otp, Long officeId) {
			final FinancialTypeMapper financialTypeMapper = new FinancialTypeMapper();
		
				StringBuilder sqlBuilder = new StringBuilder(200);
		        sqlBuilder.append("select ");
		        sqlBuilder.append(financialTypeMapper.financialTypeSchemaForOtp() + " left join m_client AS cl ON cl.id = ft.client_id where transType like '%" + type + "%'");
		        
		        if(otp != null){
		        	sqlBuilder.append(" and otp = '" +otp+"' ");
		        	sqlBuilder.append(" and paymentStatus = 0 ");
		        	sqlBuilder.append(" and flag = 0 ");
		        }
		...
		private static final class FinancialTypeMapper implements
			...
			public FinancialTransactionsData mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
				...
				return new FinancialTransactionsData(officeId, transactionId, transDate, transactionType, debitAmount, creditAmount, null, 
							userName, transactionCategory, flag, null, null, clientName, rate,status, note, clientId, officeName);
			}
			
			public String financialTypeSchema() {

				return " SQL_CALC_FOUND_ROWS ft.username as username,ft.TransId as TransId,ft.TransType as TransType,ft.Dr_amt as DebitAmount,ft.Cr_amt as CreditAmount,"
						+"ft.tran_type as tran_type,ft.flag as flag,ft.TransDate as TransDate, ft.clientName as clientName, ft.rate as rate, ft.paymentStatus as status, ft.note as note, ft.client_id as clientId, ft.office_name as officeName, ft.office_id as officeId from fin_trans_vw as ft where client_id=? ";

			}
			
			public String financialTypeSchemaByOffice() {
			...
			public String financialTypeSchemaForOtp() {

				return " SQL_CALC_FOUND_ROWS ft.username as username,ft.TransId as TransId,ft.TransType as TransType,ft.Dr_amt as DebitAmount,ft.Cr_amt as CreditAmount,"
						+"ft.tran_type as tran_type,ft.flag as flag,ft.TransDate as TransDate, ft.clientName as clientName, ft.rate as rate, ft.paymentStatus as status, ft.note as note, ft.client_id as clientId, ft.office_name as officeName, ft.office_id as officeId from fin_trans_vw as ft ";
