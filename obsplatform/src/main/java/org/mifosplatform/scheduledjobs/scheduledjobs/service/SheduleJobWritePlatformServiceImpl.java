...
@Autowired
public SheduleJobWritePlatformServiceImpl(final ChargingCustomerOrders invoiceClient,final FromJsonHelper fromApiJsonHelper,
...
try
	...
	JobParameterData data=this.sheduleJobReadPlatformService.getJobParameters(JobName.CHARGE.toString());
		if(data!=null){
			...
			String path=FileUtils.generateLogFileDirectory()+ JobName.CHARGE.toString() + File.separator +"Invoice_"+DateUtils.getLocalDateOfTenant().toString().replace("-","")+
					"_"+dateTime+".log";
						...
						for (Long clientId : clientIds) {
							try {
								if("Y".equalsIgnoreCase(data.isDynamic())){
									BillItem  invoice=this.invoiceClient.invoicingSingleClient(clientId,DateUtils.getLocalDateOfTenant());	
									fw.append("ClientId: "+clientId+"\tAmount: "+invoice.getInvoiceAmount().toString()+"\r\n");
								
								}else{
									BillItem invoice=this.invoiceClient.invoicingSingleClient(clientId,data.getProcessDate());
									fw.append("ClientId: "+clientId+"\tAmount: "+invoice.getInvoiceAmount().toString()+"\r\n");	
								}
							} catch (Exception dve) {
		...
		if(data.getUpdateStatus().equalsIgnoreCase("Y")){ 
			if(!provisioningRequests.isEmpty()){
				...
				for (ProvisioningRequest provisioningRequest : provisioningRequests) {
					fw.append("simulator Process Request id="+provisioningRequest.getId()+" ,Provisiong System="
...
try {
	...
	JobParameterData data=this.sheduleJobReadPlatformService.getJobParameters(JobName.GENERATE_BILLING.toString());
		...
		if(data!=null){
			...
			LocalTime date=new LocalTime(zone);
			String dateTime=date.getHourOfDay()+"_"+date.getMinuteOfHour()+"_"+date.getSecondOfMinute();
			String path=FileUtils.generateLogFileDirectory()+ JobName.GENERATE_BILLING.toString() + File.separator +"statement_"+DateUtils.getLocalDateOfTenant().toString().replace("-","")+"_"+dateTime+".log";
			File fileHandler = new File(path.trim());
			fileHandler.createNewFile();
			FileWriter fw = new FileWriter(fileHandler);
			FileUtils.BILLING_JOB_PATH=fileHandler.getAbsolutePath();
...
public void reportStatmentPdf() {
	try {
		...
		JobParameterData data=this.sheduleJobReadPlatformService.getJobParameters(JobName.MAKE_INVOICE.toString());
		...
		if(data!=null){
			...
			LocalTime date=new LocalTime(zone);
			String dateTime=date.getHourOfDay()+"_"+date.getMinuteOfHour()+"_"+date.getSecondOfMinute();
			String path=FileUtils.generateLogFileDirectory()+ JobName.MAKE_INVOICE.toString() + File.separator +"statement_"+DateUtils.getLocalDateOfTenant().toString().replace("-","")+"_"+dateTime+".log";
			File fileHandler = new File(path.trim());
			fileHandler.createNewFile();
			FileWriter fw = new FileWriter(fileHandler);
			FileUtils.BILLING_JOB_PATH=fileHandler.getAbsolutePath();
