	...
	private class ItemDetailsMapper implements RowMapper<ItemDetailsData>{
		...
		public ItemDetailsData mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			return new ItemDetailsData(id,itemMasterId,serialNumber,grnId,provisioningSerialNumber,quality,status,warranty,remarks,itemDescription,
					supplier,clientId,officeName,accountNumber, units, quantity,isPairing,pairedItemId,pairedItemCode,itemModel);
		}
...
/*	@Override
	...
	@Override
	public List<String> retriveSerialNumbersOnKeyStroke(final Long oneTimeSaleId, final String query,final Long officeId, Long pairedItemId) {
	...
	"(select model from b_item_attribute where id = item.item_model) as itemModel, item.remarks as remarks, master.item_description as itemDescription, " +
	...
	private class ItemDetailsMapper2 implements RowMapper<ItemDetailsData>{
		...
		return new ItemDetailsData(id, itemMasterId, serialNumber, grnId, provisioningSerialNumber, quality, status, warranty, remarks,
				itemDescription, supplier, clientId, officeName, null, null, null,null,null,null,itemModel);
		}
	...
	public ItemData retriveItemDetailsDataBySerialNum(final String query,final Long clientId,final Long officeId) {
	  ...
	  try{
			...
			String type = (this.jdbcTemplate.queryForObject("select m.selector_description from b_item_master m join b_item_detail md on md.item_master_id=m.id"
					+ " and md.serial_no = '"+query+"'", String.class));
			if(!type.isEmpty()){
			...
			final String sql = "SELECT m.id AS id, m.item_code AS itemCode, m.item_description AS itemDescription,"
					...
					+ " m.unit_price, (select price from b_item_price where region_id = "+type+" and item_id = m.id))) "
					+ " as unitPrice, itd.is_pairing As isPairing, itd.paired_item_id as pairedItemId, "
					+ " itd.provisioning_serialno as provisioningSerialNo FROM b_item_detail itd, b_item_master m"
					+ " WHERE itd.serial_no = ? AND itd.client_id IS NULL and m.id = itd.item_master_id and itd.office_id ="+officeId+" "
	...
	private class ItemMastersDataMapper implements RowMapper<ItemData>{
		...
		public ItemData mapRow(final ResultSet rs, final int rowNum)throws SQLException {
			...
			return new ItemData(id,itemCode,itemDescription,chargeCode,unitPrice,isPairing,pairedItemId,provisioningSerialNo);
		}
	}
