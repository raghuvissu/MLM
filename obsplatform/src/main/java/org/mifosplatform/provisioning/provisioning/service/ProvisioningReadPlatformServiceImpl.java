...
public class ProvisioningReadPlatformServiceImpl implements ProvisioningReadPlatformService {
	...
	public List<ProvisioningData> getProvisioningData(Long clientServiceId) {
		try {
			...
			if(clientServiceId != null){
				sql = sql+" JOIN b_service_parameters sp on sp.clientservice_id = "+clientServiceId+" AND sp.parameter_value = p.provisioning_system";
			}
	...
	private static final class ProvisioningMapper implements
		...
		public String schema() {
			return " p.id as id,p.provisioning_system as provisioningSystem,mcv.code_value as provisioningSystemName,p.command_name as CommandName,p.status as status from b_command p "
					+ "Join m_code_value mcv on mcv.id = p.provisioning_system ";
		}
	...
	private static final class ProvisioningCommandMapper implements
		...
		public String schema() {
			return " c.id as id, c.command_param as commandParam,c.param_type as paramType,c.param_length as paramLength,c.param_default as paramDefault from b_command_parameters c where c.command_id=? ";
		}
		...
		public ProvisioningCommandParameterData mapRow(final ResultSet rs,
			...
			return new ProvisioningCommandParameterData(id, commandParam, paramType, paramLength, paramDefault);
		}
	}
