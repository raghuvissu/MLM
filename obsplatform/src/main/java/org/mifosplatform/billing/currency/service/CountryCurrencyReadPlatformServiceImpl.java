	...
	private static final class CurrencyMapper implements
		...
		public String schema() {
			return " c.id as id,c.currency as currency,c.status as status,c.base_currency as baseCurrency, "
				+  " c.conversion_rate as conversionRate,c.country_isd as countryISD, c.valid_from as validFrom,c.valid_to as validTo FROM b_country_currency c ";
		...
		public CountryCurrencyData mapRow(final ResultSet rs, final int rowNum)
			...
			return new CountryCurrencyData(id,country,currency, baseCurrency,conversionRate, status, countryISD, validFrom,validTo);
		}
	}
