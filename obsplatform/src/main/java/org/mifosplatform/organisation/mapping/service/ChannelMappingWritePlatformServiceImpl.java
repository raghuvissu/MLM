...
public class ChannelMappingWritePlatformServiceImpl implements ChannelMappingWritePlatformService {
	...
	@Override
	public CommandProcessingResult deleteChannelMapping(Long channelmappingId) {
		try{
			this.context.authenticatedUser();
			ChannelMapping channelmapping = this.retrieveChannelMapping(channelmappingId);
			if(channelmapping.getIsDeleted()=='Y'){
				throw new ChannelMappingNotFoundException(channelmappingId);
			}
			channelmapping.delete();
			this.channelmappingRepository.saveAndFlush(channelmapping);
			return new CommandProcessingResultBuilder().withEntityId(channelmappingId).build();
