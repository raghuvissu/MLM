	...
	private NewCommandSourceHandler findCommandHandler(final CommandWrapper wrapper) {
		...
		if (wrapper.isConfigurationResource()) {
	       ...
	       if (wrapper.isCreate()) {
	        ...
	        }else if (wrapper.isConfirmPayment()) {
	        	handler = applicationContext.getBean("confirmPaymentCommandHandler", NewCommandSourceHandler.class);
	      ...
	      }else if (wrapper.isReferalResource()) {
				if (wrapper.isCreate()) {
					handler = applicationContext.getBean("createReferalCommandHandler",NewCommandSourceHandler.class);
				} else if (wrapper.isUpdate()) {
					handler = applicationContext.getBean("updateReferalCommandHandler",NewCommandSourceHandler.class);
				} else {
					throw new UnsupportedCommandException(wrapper.commandName());
				}
			}else if (wrapper.isEntityPaymentResource()) {
				if (wrapper.isCreate()) {
					handler = applicationContext.getBean("entityPaymentCommandHandler",NewCommandSourceHandler.class);
				} else {
					throw new UnsupportedCommandException(wrapper.commandName());
				}
			}else if (wrapper.isCommissionPaymentResource()) {
				if (wrapper.isCreate()) {
					handler = applicationContext.getBean("createCommissionPaymentCommandHandler",NewCommandSourceHandler.class);
				} else {
					throw new UnsupportedCommandException(wrapper.commandName());
				}
			}else if (wrapper.isCommissionWithdrawPaymentResource()) {
				if (wrapper.isCreate()) {
					handler = applicationContext.getBean("createCommissionWithdrawPaymentCommandHandler",NewCommandSourceHandler.class);
				} else {
					throw new UnsupportedCommandException(wrapper.commandName());
				}
			}else {
