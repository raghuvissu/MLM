 ...
 + " where o.hierarchy like ? and pgc.group_id = ?";
    ...
    private static final class ClientMembersOfGroupMapper implements RowMapper<ClientData> {
        ...
        public ClientData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            return ClientData.instance(accountNo,groupName, status, officeId, officeName, id, firstname, middlename, lastname, fullname, displayName,
                    ...
                    entryType,walletAmount,null,null,null,title,paidAmount,lastBillAmount,lastPaymentDate, null);
        }
    }
    ...
    private static final class ClientMapper implements RowMapper<ClientData> {
        ...
        public ClientMapper() {
           ...
           //builder.append(" a.zip as zipcode,b.balance_amount as balanceAmount,b.wallet_amount as walletAmount,bc.currency as currency,");
            builder.append(" a.zip as zipcode,b.balance_amount as balanceAmount,b.wallet_amount as walletAmount,bc.code as currency, rm.external_code as externalCode, ");
            builder.append(" coalesce(min(ba.serial_no),min(oh.serial_number),'No Device') HW_Serial, ");
            builder.append(" b.wallet_amount As walletAmount ,bp.amount_paid AS paidAmount, bp.payment_date as lastPaymentDate,bbm.Due_amount AS lastBillAmount, c.bill_mode as billMode ");
            builder.append(" from m_client c ");
            builder.append(" join m_office o on o.id = c.office_id ");
            builder.append(" left join m_referal_master rm on rm.external_id = c.id ");
           ...
           //builder.append(" left outer join b_country_currency bc on  bc.country = a.country ");
            builder.append(" left outer join m_currency bc on  bc.country_code = a.country ");
            ...
            builder.append(" left outer join b_payments bp ON bp.client_id = c.id and bp.id=(select max(bsp.id) from b_payments bsp where bp.client_id=bsp.client_id) ");
            builder.append(" left outer join b_bill_master bbm ON bbm.client_id = c.id and bbm.id=(select max(bbsm.id) from b_bill_master bbsm where bbm.client_id=bbsm.client_id) ");
        ...
        public ClientData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            final String externalCode=rs.getString("externalCode");
           
            ClientData clientData = ClientData.instance(accountNo,groupName, status, officeId, officeName, id, firstname, middlename, lastname, fullname, displayName,
                    externalId, activationDate, imageKey,categoryType,email,phone,homePhoneNumber, addressNo, street, city, state, country, zipcode,
                    clientBalance,hwSerial,currency,taxExemption,entryType,walletAmount,userName,clientpassword,parentId,title,paidAmount,lastBillAmount,lastPaymentDate,
                    externalCode);
	...
	public Collection<ClientCategoryData> retrieveClientCategories() {
       ...
       private static final class parentClientMapper implements RowMapper<ClientCategoryData> {
		...
		@Override
		public Collection<ClientData> retrieveAllClientsList() {
	        ...
	        return this.jdbcTemplate.query(sqlBuilder.toString(), clientMapper , new Object[] { hierarchySearchString });
		...
		private class ClientResourceMapper implements RowMapper<ClientData> {
			...
			public ClientData mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				return ClientData.instance(accountNo,null, status, null, officeName, id, null, null, null, null, displayName,
	                    ...
	                    clientBalance,hwSerial,null,null,null,walletAmount,userName,clientpassword,null,null,paidAmount,lastBillAmount,lastPaymentDate, null);
			}
		...
		public ClientData retrieveOne(String columnName, String columnValue) {
	        ...
	        try {
	        	...
	        	final String hierarchySearchString = hierarchy + "%";
	            final ClientMapper clientMapper = new ClientMapper();
	            final String sql = "select " + clientMapper.schema() + " where o.hierarchy like ? and c."+columnName+" = '"+columnValue+"' ";
	            return this.jdbcTemplate.queryForObject(sql,clientMapper,new Object[] { hierarchySearchString });
	           /* final String clientGroupsSql = "select " + this.clientGroupsMapper.parentGroupsSchema();

	            final Collection<GroupGeneralData> parentGroups = this.jdbcTemplate.query(clientGroupsSql, this.clientGroupsMapper,new Object[] {});
	            return ClientData.setParentGroups(clientData, parentGroups);*/
	        } catch (EmptyResultDataAccessException e) {
		...
		private static final class ClientDataMapper implements RowMapper<ClientData> {
				...
				public ClientData mapRow(ResultSet resultSet, int rowNum) throws SQLException {
					...
					return ClientData.temperory(groupName, status, officeId, officeName, id, activationDate, imageKey,poId,accountNo);
		...
		@Override
		public Page<ClientData> retrieveAllClientsForLCO(SearchParameters searchParameters) {
			


	        final AppUser currentUser = context.authenticatedUser();
	        final LCOResourceMapper lcoMapper = new LCOResourceMapper();
	        final String hierarchy = currentUser.getOffice().getHierarchy();
	        ...
	        sqlBuilder.append(lcoMapper.schema());
	        sqlBuilder.append(" Where co.order_status=1 and alloc.status='allocated' and im.item_class=1 and  o.hierarchy like '%.%' order by 1");
		...
		private class LCOResourceMapper implements RowMapper<ClientData> {
			...
			return "Select c.id as id,co.id as orderId,alloc.serial_no as stbId,c.account_no as accountNo,"+
		...
		public ClientData mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			if(startDate==null){
				...
				endDate = org.apache.commons.lang.time.DateUtils.addMonths(startDate, 1);
			}
			
			return ClientData.lcoClient(id,accountNo,displayName,phone,clientBalance,orderId,stbId,startDate,endDate);
			}
	}
}
