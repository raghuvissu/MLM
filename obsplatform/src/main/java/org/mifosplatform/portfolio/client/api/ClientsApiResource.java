...
public class ClientsApiResource {
    ...
    public ClientsApiResource(final PlatformSecurityContext context, final ClientReadPlatformService readPlatformService,
            ...
            final ToApiJsonSerializer<ClientAdditionalData> jsonSerializer,final ClientBillInfoReadPlatformService clientBillInfoReadPlatformService,
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String retrieveOne(@PathParam("clientId") final Long clientId, @Context final UriInfo uriInfo) throws JSONException {
	...
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public String retriveSearchOffice(@PathParam("officeId") final Long officeId, @QueryParam("query") final String query, @Context final UriInfo uriInfo){
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveClientBillInfoDetails(@Context final UriInfo uriInfo ,@PathParam("clientId") final Long clientId) {
		
		this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
		ClientBillInfoData clientBillData = this.clientBillInfoReadPlatformService.retrieveClientBillInfoDetails(clientId);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
		if(settings.isTemplate()){
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String retrieveCustomer(@QueryParam("key") final String key,@QueryParam("value") final String value, @Context final UriInfo uriInfo) throws JSONException {
    	this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
