 ...
 public OrderDetailsReadPlatformServicesImpl(final TenantAwareRoutingDataSource dataSource) {
	...
	private static final class PlanMapper implements RowMapper<ServiceData> {
		...
		public String schema() {
			...
			return "da.id as id,se.id as productId, da.product_id as product_id,se.product_code as product_code,se.service_id as serviceType,da.plan_id as plan_code "
		...
		public ServiceData mapRow(final ResultSet rs,final int rowNum) throws SQLException {
			...
			return new ServiceData(id,productId,productCode,null, null,null,null, null,serviceType,null,null,null);
	}
	}
	@Override
		public List<PriceData> retrieveAllPrices(Long planId,String billingFreq, Long clientId ) {
			...
			String sql ="SELECT da.id AS id,if(da.product_id = 'None', 0, se.id) AS productId,da.product_id AS product_id,da.charge_code AS charge_code," +
		...
		private static final class PriceMapper implements RowMapper<PriceData> {
			...
			public String schema() {
				return " da.id AS id,if(da.product_id ='None',0, se.id) AS productId, da.product_id AS product_id,da.charge_code AS charge_code,da.charging_variant AS charging_variant," +
						...
						"pd.country_id AS regionCountryId FROM b_plan_pricing da,b_charge_codes c,b_product se,b_client_address ca,b_state s,b_country con,b_priceregion_detail pd,b_priceregion_master prd" +
			...
			public PriceData mapRow(final ResultSet rs,final int rowNum) throws SQLException {
				...
				return new PriceData(id, productId, chargeCode,chargingVariant,price,chargeType,chargeDuration,durationType,
			              null,discountId,taxinclusive,stateId,countryId,regionState,regionCountryId);
