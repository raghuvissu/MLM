	...
	try{
		...
		//if(order.getNextBillableDay() != null){
			...
			billingOrderDatas.add(new BillingOrderData(orderId,addons.getPriceId(),order.getPlanId(),order.getClientId(),startDate.toDate(),
					...
					order.getStatus(),orderPrice.isTaxInclusive()?1:0,String.valueOf(client.getTaxExemption()),false));
			...
			this.chargingOrderWritePlatformService.updateClientBalance(invoice.getInvoiceAmount(),order.getClientId(),false);
