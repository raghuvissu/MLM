		...
		AppUser user = context.authenticatedUser();
		  String sql = null;
            ...
            "+user.getId()+"*/
            	...
            	sql="SELECT DISTINCT s.id AS id, s.plan_code AS planCode, s.plan_description as planDescription, s.is_prepaid AS isPrepaid" +
                    	 " FROM b_plan_master s, b_plan_pricing p,b_priceregion_master prd,b_priceregion_detail pd,b_client_address cd,b_state bs" +
                    	 " WHERE  s.plan_status = 1 AND s.is_deleted = 'n' AND s.id != "+planId+"  AND prd.id = pd.priceregion_id AND p.price_region_id = pd.priceregion_id AND " +
                    	 " (pd.state_id = ifnull((SELECT DISTINCT c.id FROM b_plan_pricing a,b_priceregion_detail b,b_state c,b_charge_codes cc,b_client_address d" +
                    	 " WHERE  b.priceregion_id = a.price_region_id AND b.state_id = c.id AND a.price_region_id = b.priceregion_id AND d.state = c.state_name " +
                    	 ...
                    	 " b_charge_codes cc,b_client_address d WHERE b.priceregion_id = a.price_region_id AND b.country_id = c.id AND cc.charge_code = a.charge_code" +
                    	 " AND cc.charge_code = p.charge_code AND a.price_region_id = b.priceregion_id AND c.country_name = d.country AND d.address_key = 'PRIMARY'" +
                    	 " AND d.client_id = "+clientId+"  AND a.plan_id != ? AND a.is_deleted = 'n'),0)) AND s.id = p.plan_id AND cd.client_id = "+clientId+" "
                    	 		+ " AND p.plan_id not in (Select distinct plan_id from b_orders where order_status=1 and client_id= "+clientId+") " ;
                    	;
            	if(!user.isSuperUser()){
            		sql = sql +" AND p.plan_id in (select plan_id from b_sales_cataloge_mapping scm"+
            				" left join b_sales_cataloge sc on sc.id=scm.cataloge_id" + 
                        	" left join b_user_cataloge uc on uc.cataloge_id=sc.id" + 
                        	" where uc.user_id = "+user.getId()+" and uc.is_deleted='N' and scm.is_deleted='N')" +
                        	 " GROUP BY s.id";
            	}
	...
	public List<OrderPriceData> retrieveOrderPriceData(Long orderId) {
	 ...
	 private static final class OrderPriceMapper implements RowMapper<OrderPriceData> {
		 public String schema() {
			return  "s.id as id,s.order_id as order_id,s.charge_code as charge_code,s.product_id as product_id,s.charge_type as charge_type,s.charge_duration as charge_duration,"
	        ...
	        public OrderPriceData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
			  ...
			  Long productId = rs.getLong("product_id");
	            ...
	            return new OrderPriceData(id,orderId,productId,chargeCode,chargeType,chargeDuration,durationtype,price, null, null, null, null,null,null,null);
	        }
	...
	private static final class OrderPriceDataMapper implements RowMapper<OrderPriceData> {
        ...
        public OrderPriceData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
	          ...
	          LocalDate invoiceTillDate=JdbcSupport.getLocalDate(rs,"invoiceTillDate");
	            ...
	            return new OrderPriceData(id,orderId,clientId,productDescription,chargeDescription,chargeDuration,durationtype,price,billStartDate,
	            		billEndDate,nextBillDate,invoiceTillDate,billingAlign,billingFrequency,null);
}
}
				...
				try {
					...
					private static final class ClientOrderServiceMapper implements RowMapper<OrderLineData> {
						...
						return " ol.id AS id,p.id AS productId,ol.order_id AS orderId,p.product_code AS productCode,s.is_auto AS isAuto," +
								" p.product_description AS productDescription,s.service_type AS serviceType, psd.image AS image " +
								" FROM b_order_line ol, b_product p,b_service s left join b_prov_service_details psd on  psd.service_id = s.id  " +
								" WHERE order_id = ? AND ol.product_id = p.id AND ol.is_deleted = 'N'";
						}
						...
						return new OrderLineData(id,orderId,productCode,productDescription,serviceType,productId,isAutoProvision,image);
						}
				}
